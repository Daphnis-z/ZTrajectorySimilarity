...
public class DTWSimilarity implements SimilarityReactor{
	...
	private void DTWNoTime(Trajectory objTraj,Trajectory testTraj){
		...
		{
			if((y>=1)&&(avr_dtw_dis-dtw_dis[x][y-1]<=0.00001))
			{
			...
			else if((x>=1)&&(avr_dtw_dis-dtw_dis[x-1][y]<=0.000001))
			{
			...
			else if((x>=1&&y>=1)&&(avr_dtw_dis-dtw_dis[x-1][y-1]<=0.000001))
			{	
		...
		{
			System.out.println(tip[i][0]+"<--->"+tip[i][1]+" "+test_time[tip[i][0]]+"<--->"+obj_time[tip[i][0]]);
