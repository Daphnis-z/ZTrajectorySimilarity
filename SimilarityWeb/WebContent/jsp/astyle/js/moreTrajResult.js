...
//绘制轨迹
function drawTraj(strTrajs,allTrajs,hasTime){
	if($("select").eq(0).val()=="map"){
		if($('option:selected', '#trajs').index()==0){
			showMapWithSubtraj(strTrajs,strSubtrajs,strPoints,hasTime);
		}else{
			showMap(strTrajs,hasTime);
		}
	}else{
		showInChart(allTrajs,hasTime);
	}
}
