...
public class SimilarityWithoutTime extends Similarity{
	...
	public void calculate_DSE(){//计算距离、形状、编辑距离
	...
	public void calculate_SimilarerMatch(){//计算最相似点与段
		...
		for(int i=1;i<=x;i++){
			for(int j=1;j<=y;j++){
				if(raw_dis[i-1][j-1]<=md/*&&Math.abs(i-j)<=me*/){
...
//				}
				...
				similarestTraj[0]=trajs.elementAt(0)[0];
				similarestTraj[1]=trajs.elementAt(0)[1];
			}
			else{
				if(similarestPoint[0].pid>0&&similarestPoint[1].pid>0){
		    		double lat=(similarestPoint[0].getLatitude()+testTraj.getPoints().elementAt(similarestPoint[0].pid-1).getLatitude())/2;
	...
	public double subsimilarity(Trajectory t1,Trajectory t2){
		...
		for(int i=0;i<t1.getSize();i++){
			...
			//sube+=Math.abs(t1.getPoints().elementAt(i).pid-t2.getPoints().elementAt(i).pid);
		}
		subd/=t1.getSize();
		//sube/=t1.getSize();
		subs=sDef.getDtwDis_W()*(1-subd/dis)+sDef.getEditDis_W()+sDef.getShapeSum_W();
